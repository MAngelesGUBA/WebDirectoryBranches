    ...
    return `
      <tr class='editable-row' id='${extension.id}'>
        ...
        <td class='action-column action-buttons' data-title='Acciones'>
          <button class='btn-update-data' type='button' disabled>
          ...
          <button class='btn-delete-data' type='button' disabled>
  ...
  try {
    ...
    document.getElementById('frmSearch').addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const formData = new FormData(event.target);
        let plainData= Object.fromEntries(formData); 
        const url = `${API_ENDPOINTS.getExtension}?${new URLSearchParams(plainData)}`;
